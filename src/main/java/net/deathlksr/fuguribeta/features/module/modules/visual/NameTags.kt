...
object NameTags : Module("NameTags", Category.VISUAL, hideModule = false) {
    ...
    private fun renderNameTag2D(entity: EntityLivingBase) {
        ...
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        val text = ownerText + userText + penisBonan + friendText + entity.displayName.formattedText
        ...
        if (shadow) {
            RenderUtils.drawShadow((-stringWidth - 1f), -14f, (stringWidth * 2f) + 2f, 10f)
        }
        val offset: Float = (fontRenderer.FONT_HEIGHT - 22f)
