...
object TargetHud : Module("TargetHud", Category.VISUAL, gameDetecting = false, hideModule = false) {
        ...
        private fun getPlayerName(entity: EntityLivingBase): String {
            ...
            var pre = ""
            val teams = FuguriBeta.moduleManager[Teams::class.java]!!
            if (FuguriBeta.fileManager.friendsConfig.isFriend(entity.name)) {
                pre = "$pre§b[Friend] "
            }
            ...
            if (!AntiBot.isBot(entity) && !teams.isInYourTeam(entity)) {
                pre = if (FuguriBeta.fileManager.friendsConfig.isFriend(entity.name)) {
                    "§b[Friend] §c"
                } else {
                    "§c"
                }
