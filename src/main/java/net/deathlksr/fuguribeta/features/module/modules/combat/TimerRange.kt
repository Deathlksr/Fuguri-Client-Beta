...
import net.deathlksr.fuguribeta.utils.RaycastTimerRange
import net.deathlksr.fuguribeta.utils.extensions.expands
import net.deathlksr.fuguribeta.utils.extensions.getNearestPointBB
...
object TimerRange : Module("TimerRange", Category.COMBAT, hideModule = false) {
    ...
    private val eventStates = ListValue("EventState", arrayOf("PRE", "POST"), "PRE")
    ...
    fun onMotion(event: MotionEvent) {
        ...
        if (event.eventState == if (eventStates.get() == "POST") EventState.POST else EventState.PRE) return // post event mean player's tick is done
        val thePlayer = mc.thePlayer ?: return
        if (onlyKillAura.get() && !killAura.state) return
        if (mode.get() == "RayCast") {
            val entity =
                RaycastTimerRange.raycastEntity(maxDistance.get() + 1.0
                ) { entity -> entity != null && entity is EntityLivingBase && (!onlyPlayer.get() || entity is EntityPlayer) }
    ...
    fun handleTick(): Boolean {
        ...
        if (!reverseFreeze) {
            ...
            if (reverseAuraClicks.get()) {
                if (killAura.target?.canAttackWithItem() == true && reverseAuraClick.get() == "BeforeTeleport") {
                    killAura.clicks += 1
                }
                ++freezeTicks
                mc.runTick()
