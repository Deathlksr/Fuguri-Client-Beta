...
import net.deathlksr.fuguribeta.handler.api.Status
...
import net.deathlksr.fuguribeta.utils.misc.HttpUtils.get
...
object SettingsCommand : Command("autosettings", "autosetting", "settings", "setting") {
    ...
    private suspend fun uploadSettings(args: Array<String>) {
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val name = "${FuguriBeta.clientCommit}-${serverData.serverIP.replace(".", "_")}"
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                loadSettings(false) { settingsList ->
                    ...
                    val configFiles = FuguriBeta.fileManager.settingsDir.listFiles { _, name ->
                        name.endsWith(".txt")
                    }
    ...
    private suspend fun deleteConfig(args: Array<String>) {
        withContext(Dispatchers.IO) {
            ...
            val file = File(FuguriBeta.fileManager.settingsDir, "$configName.txt")
    ...
    private suspend fun openFolder() {
        withContext(Dispatchers.IO) {
            Desktop.getDesktop().open(FuguriBeta.fileManager.settingsDir)
            chat("Opening folder...")
        }
    }
    ...
    private suspend fun renameConfig(args: Array<String>) {
        withContext(Dispatchers.IO) {
            ...
            val oldFile = File(FuguriBeta.fileManager.settingsDir, "$oldName.txt")
            val newFile = File(FuguriBeta.fileManager.settingsDir, "$newName.txt")
    ...
    private suspend fun currentConfig() {
        withContext(Dispatchers.IO) {
            alert("Current config is ${FuguriBeta.fileManager.nowConfig}")
