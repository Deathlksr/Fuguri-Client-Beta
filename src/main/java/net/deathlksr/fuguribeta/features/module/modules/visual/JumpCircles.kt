...
import net.deathlksr.fuguribeta.utils.render.RenderUtils.setupDrawCircles
...
import net.deathlksr.fuguribeta.utils.render.animation.AnimationUtil.easeInOutElasticx
import net.deathlksr.fuguribeta.utils.render.animation.AnimationUtil.easeInOutExpo
import net.deathlksr.fuguribeta.utils.render.animation.AnimationUtil.easeOutCirc
import net.deathlksr.fuguribeta.utils.render.animation.AnimationUtil.easeWave
...
object JumpCircles : Module("JumpCircles", Category.VISUAL, hideModule = false) {
    ...
    private val maxTime by IntegerValue("MaxTime", 3000, 2000..8000)
    private val radius by FloatValue("Radius", 2f, 1f..3f)

    private val texture by ListValue("Texture", arrayOf("Supernatural", "Aurora", "Leeches", "Circle"), "Leeches")
    private val deepestLight by BoolValue("GlowLight", true)
    private val glowIntensity by FloatValue("GlowIntensity", 0.1f, 0f.. 0.5f) { deepestLight }
    ...
    private val staticLoc = ResourceLocation("${CLIENT_NAME.lowercase()}/jumpcircles/default")
    private val animatedLoc = ResourceLocation("${CLIENT_NAME.lowercase()}/jumpcircles/animated")
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        val immersiveStrength = when {
            deepestLightAnimation >= 1f / 255f -> {
                when (texture) {
                    "Circle", "Emission" -> glowIntensity
                    "Supernatural", "Aurora", "Inusual" -> glowIntensity
                    "Leeches" -> glowIntensity
                    else -> 0f
                }
            }
            else -> 0f
